...
#         basche & delonge used porosity as a measure of soil 'health'
#
#
##############################
...
#--look at it, britt's method is always higher
dat_poros  %>% 
...
dat_poros %>% 
  ...
  ggplot(aes(cc_trt, poros_mineral)) + 
  geom_point() + 
  stat_summary(fun.y = "mean", geom = "point", color= "red", size = 4) +
  stat_summary(fun.data = "mean_se", geom = "linerange", color= "red") +
  facet_grid(.~site_sys)
...
tidy_data %>% write_csv("data/tidy/td_water-curves.csv")

tidy_boyd <- 
  tidy_data %>% 
  filter(str_detect(site_trt, "Boyd")) 

tidy_boyd %>% write_csv("data/tidy/td_water-curves-boyd.csv")
...
ggsave("figs/boyd-wrc-means.png")
